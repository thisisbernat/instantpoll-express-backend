...
router.post('/signup', (req, res, next) => {
	...
	if (email === '' || password === '' || firstname === '' || lastname === '' || username === '') {
		res.status(400).json({ message: 'Provide email, password, username and both names' });
		return;
	}
	...
	User.findOne({ email })
		.then((foundUser) => {
			...
			return User.create({ email, password: hashedPassword, firstname, lastname, username });
		})
		.then((createdUser) => {
